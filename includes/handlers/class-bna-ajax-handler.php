...
class BNA_Ajax_Handler {
    ...
    public function complete_payment() {
        error_log('BNA NEW: complete_payment called');
        error_log('BNA NEW: POST data: ' . print_r($_POST, true));
        
        // Check nonce
        if (!wp_verify_nonce($_POST['_ajax_nonce'] ?? '', 'bna_ajax_nonce')) {
            error_log('BNA NEW: Nonce verification failed');
        ...
        error_log('BNA NEW: Order ID: ' . $order_id . ', Transaction: ' . $transaction_id);
        
        if (!$order_id) {
            error_log('BNA NEW: Invalid order ID');
        ...
        if (!$order) {
            error_log('BNA NEW: Order not found');
        ...
        if ($order->has_status(array('completed', 'processing'))) {
            error_log('BNA NEW: Order already completed');
        ...
        try {
            ...
            error_log('BNA NEW: Order completed successfully');
            wp_send_json_success('Order completed');
            
        } catch (Exception $e) {
            error_log('BNA NEW: Error: ' . $e->getMessage());
