...
final class BNA_Payment_Bridge {
    ...
    public function init() {
        ...
        // Initialize WooCommerce integration immediately
        add_action('woocommerce_init', array($this, 'init_woocommerce'));

        // Also try to initialize if WooCommerce is already loaded
    ...
    public function init_woocommerce() {
        ...
        // Debug: Log when gateway is being registered
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[BNA Bridge] WooCommerce integration initialized');
        }
    }
    ...
    public function add_payment_gateway($gateways) {
        if (class_exists('BNA_Bridge_Gateway')) {
            ...
            // Debug: Log when gateway is added
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[BNA Bridge] Payment gateway added to WooCommerce');
            }
        } else {
            // Debug: Log if class doesn't exist
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[BNA Bridge] BNA_Bridge_Gateway class not found');
            }
        }
